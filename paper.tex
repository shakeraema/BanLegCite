...
To construct the benchmark, we first compiled a corpus of verified, real landmark case law citations from primary Bangladeshi case reporters (\textit{Dhaka Law Reports}, \textit{Bangladesh Law Chronicles}, \textit{Apex Law Reports}) and statutory codes (Penal Code 1860, CrPC, CPC, and the \textit{Nari O Shishu Nirjatan Daman Amendment Act 2026}). To introduce controlled legal anomalies without parametric bias, we employed a hybrid dataset construction pipeline combining programmatic mutations and LLM-assisted generation across two phases:
\begin{enumerate}
\item \textbf{Programmatic Offsets (P2 Locators):} For $N=46$ precedent tasks, volume and page numbers were shifted by deterministic numerical offsets ($\pm 5$ to $\pm 20$ pages/volumes) while preserving genuine case titles and reporter names.
...
\item \textbf{Phase 2 LLM Generation (15 Tasks):} Extended statutory and precedent anomalies generated using GLM-5.2 (\texttt{z-ai/glm-5.2:free}) to construct complex misattributed holdings ($P3$), repealed section anomalies ($S5$), and cross-jurisdictional bleeds ($S4$).
\end{enumerate}

\begin{table}[htbp]
\caption{Dataset Construction Mechanism by Taxonomy Category ($N=150$)}
...
We executed a double-blind annotation workflow involving two independent law graduate annotators—$A_1$ (Bushra Hakim, Undergraduate Student, Leading University) and $A_2$ (Haris Rahman Antor, Law Graduate, Leading University)—and senior legal adjudicator Shammi Akther (Assistant Law Officer, RAJUK).
...
To prevent self-recognition bias, verifier models were selected to be strictly disjoint from the two models used during dataset fabrication generation (Gemini 3.5 Flash, GLM-5.2). We report results for all seven evaluated candidate models (\texttt{google/gemini-2.5-flash-lite}, \texttt{openai/gpt-4o-mini}, \texttt{deepseek/deepseek-chat}, \texttt{meta-llama/llama-3.3-70b-instruct}, \texttt{qwen/qwen-2.5-72b-instruct}, \texttt{deepseek-v4-flash}, and \texttt{glm-5.3}) across both Standard and Agentic RAG settings (Table \ref{tab:full_suite_metrics}). For post-hoc interpretive clarity, we group these seven models into two behavioral tiers observed in the empirical results: (1) five models exhibiting graduated, bidirectional RAG performance effects (improvements up to +8.00\% or moderate degradations down to -9.34\%), analyzed as our primary comparison suite (Table \ref{tab:metrics}), and (2) two models exhibiting extreme, categorical failure modes—namely, a severe context-collapse ($85.33\% \rightarrow 56.00\%$, $\chi^2 = 35.56, p < 0.0001$) and a degenerate constant-prediction prior ($50.67\%$, $\chi^2 = 0.0, p = 1.0$), presented separately in Section \ref{sec:diagnostic}. This grouping is descriptive of the observed results rather than a pre-registered selection criterion, ensuring complete transparency across the full evaluated pool.
...
\item \textbf{Formal Retrieval Match Criterion:} Retrieval was scored as successful for a given task if the top-1 retrieved chunk ($R_i$) contained the authoritative citation identifier (Act/Section number for statutes, or Case Title + Reporter/Volume/Page locator for precedents) corresponding to the task's ground-truth source document, verified via string exact-match verification over the indexed primary corpus. Under this criterion, the retriever achieved 100.00\% retrieval coverage ($150/150$ tasks).
...
\subsection{Pre-Registered Statistical Tests \& Multiple Comparisons}
...
\item \textbf{Hypothesis 1 (McNemar's Test):} Checks if LLMs under standard prompting perform significantly differently from the agentic setting. McNemar's test is a non-parametric test applied to $2 \times 2$ contingency tables with a paired design:
   ...
   \end{equation}
   where $b$ represents the number of tasks where standard prompting is incorrect but agentic RAG is correct, and $c$ represents the number of tasks where standard prompting is correct but agentic RAG is incorrect. We report the raw discordant-pair counts ($b, c$), $\chi^2$ statistics, and $p$-values for all models in Table~\ref{tab:stats}.
...
\toprule
\textbf{Model Name} & \textbf{Suite Tier} & \textbf{Standard Acc. (95\% CI)} & \textbf{Agentic Acc. (95\% CI)} & \textbf{$\Delta$ RAG Impact} & \textbf{McNemar $p$-val} \\
\midrule
